...
def test_measurement():
    distance_metric = Measurement(17, 27, 20, 'metric', 1)
    assert_equal(distance_metric.trig_pin, 17)
    assert_equal(distance_metric.echo_pin, 27)
    assert_equal(distance_metric.temperature, 20)
    assert_equal(distance_metric.unit, 'metric')
    assert_equal(distance_metric.round_to, 1)
...
def test_imperial_measurements():
    ...
    assert_equal(
        imperial_distance, round(raw_measurement * 0.394, value.round_to)
        )
    assert_equal(
        imperial_depth, round(hole_depth - (raw_measurement * 0.394), value.round_to)
        )
...
def test_metric_measurements():
    ...
    assert_equal(
        metric_distance, round(raw_measurement, value.round_to)
        )
    assert_equal(
        metric_depth, round(hole_depth - raw_measurement, value.round_to)
        )
